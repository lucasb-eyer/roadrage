...
{
    if(idx > 15)
        throw std::runtime_error("Index out of bound in Base4x4Matrix::operator[i]");
...
{
    if(i > 4 || j > 4 || i == 0 || j == 0)
        throw std::runtime_error("Index out of bound in Base4x4Matrix::operator[i,j]");
...
AffineMatrix AffineMatrix::rotationQuat(const Quaternion& in_quat)
{
    float s = 0.0f;
